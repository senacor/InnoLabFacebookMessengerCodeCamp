...
const facebookEventHandler = req => {
    ...
    const bodyEntryHandlers = objectPath.get(req, 'body.entry', []).map(entry => {
        const messageHandlers = objectPath.get(entry, 'messaging', []).map(messaging => {
            // 1) Event sent by Facebook after account linking
            if (objectPath.get(messaging, 'account_linking.status') === 'linked') {
